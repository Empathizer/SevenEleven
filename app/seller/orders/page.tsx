...
export default function SellerOrdersPage() {
  ...
  const updateStatus = async (orderId: string, status: string, requiredAmount: number) => {
    if (status === 'processing' && walletBalance !== null && walletBalance < requiredAmount) {
            ...
            {orders.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE).map((order) => {
              ...
              return (
                <TableRow key={order._id}>
                  ...
                  <TableCell>
                    <Badge className={statusColor(myItems[0]?.sellerStatus || 'pending')}>
                      {myItems[0]?.sellerStatus || 'pending'}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    {loading ? (
                      ...
                      <>
                        {!hasBalance && (myItems[0]?.sellerStatus || 'pending') === 'pending' && (
                          <p className="text-xs text-destructive mb-1">Need ${buyingCost.toFixed(2)} to pick</p>
                        )}
                        <Select 
                          value={myItems[0]?.sellerStatus || 'pending'} 
                          onValueChange={(v) => updateStatus(order._id, v, buyingCost)} 
                          disabled={(myItems[0]?.sellerStatus || 'pending') !== 'pending'}
                        >
                          <SelectTrigger className="w-32 bg-muted"><SelectValue /></SelectTrigger>
                          <SelectContent className="bg-card">
                            <SelectItem value="pending">Pending</SelectItem>
                            <SelectItem value="processing" disabled={!hasBalance}>Pick Order</SelectItem>
